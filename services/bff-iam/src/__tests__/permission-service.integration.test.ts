...
describe('RoleAccessPermissionService integration', () => {
  ...
  let service: RoleAccessPermissionService;
  ...
  beforeEach(async () => {
    ...
    await migrateAuthzDatabase(pool);
    service = new RoleAccessPermissionService(
      new RoleAccessRepository(pool),
      redis,
      {
  ...
  expect(initial.microfrontends.map((item) => item.id)).not.toContain('mfe-admin-acessos');
    ...
    const auditRows = await pool.query<{
      role: string;
      previous_value_json: { version: number; permissions: string[] };
      new_value_json: { version: number; permissions: string[] };
      changed_by: string;
