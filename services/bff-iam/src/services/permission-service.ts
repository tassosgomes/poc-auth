...
function normalizeRoles(roles: readonly string[]): { validRoles: FixedRole[]; ignoredRoles: string[] } {
  const validRoles: FixedRole[] = [];
...
function sortByFixedRoleOrder(entries: RoleAccessConfig[]): RoleAccessConfig[] {
  return [...entries].sort(
    (left, right) => FIXED_ROLES.indexOf(left.role) - FIXED_ROLES.indexOf(right.role)
  );
}

function buildRoleAccessCacheKey(role: FixedRole, version: number): string {
  return `${ROLE_ACCESS_CACHE_PREFIX}${role}:${version}`;
}

function buildPermissionSnapshotCacheKey(userId: string, sessionVersion: number): string {
  return `${PERMISSION_SNAPSHOT_PREFIX}${userId}:${sessionVersion}`;
}

function mapRoleVersions(entries: readonly Pick<RoleAccessConfig, 'role' | 'version'>[]): Record<string, number> {
  return Object.fromEntries(entries.map((entry) => [entry.role, entry.version]));
}

function sameRoleVersions(
  currentRoles: readonly FixedRole[],
...
function createDeniedSnapshot(input: PermissionLookupInput): PermissionSnapshot {
  return {
    userId: input.userId,
    roles: [],
    permissions: [],
...
function createSnapshot(userId: string, roles: readonly FixedRole[], configs: readonly RoleAccessConfig[]): PermissionSnapshot {
  ...
  const microfrontends = MICROFRONTEND_CATALOG_SEED.filter(
    (item) => authorizedMicrofrontendIds.has(item.id) && item.requiredPermissions.every((permission) => permissions.includes(permission))
  );
...
export class RoleAccessPermissionService implements PermissionService {
  constructor(
    private readonly repository: RoleAccessRepository,
    private readonly redis: AnyRedisClient,
    private readonly config: Pick<BffConfig, 'permissionSnapshotCacheTtlSeconds' | 'roleAccessCacheTtlSeconds'>,
  ...
  private async loadRoleAccessConfigs(
    roles: readonly FixedRole[],
    versions: readonly Pick<RoleAccessConfig, 'role' | 'version'>[]
    ...
    const cachedByRole = new Map<FixedRole, RoleAccessConfig>();
    const missingRoles = new Set<FixedRole>();
    ...
    return roles
      ...
      .filter((entry): entry is RoleAccessConfig => Boolean(entry));
  }

  private validateUpdateCommand(command: RoleAccessUpdateCommand): RoleAccessUpdateCommand {
    ...
    const missingRequiredPermissions = MICROFRONTEND_CATALOG_SEED.filter((item) => microfrontends.includes(item.id)).flatMap(
      (item) => item.requiredPermissions.filter((permission) => !permissions.includes(permission))
    );
  ...
  private assertAllowedValues(field: string, values: readonly string[], allowList: ReadonlySet<string>): void {
  ...
  private async safeMultiGet(keys: readonly string[]): Promise<Array<string | null>> {
    try {
  ...
  private async readPermissionSnapshotCache(key: string): Promise<CachedPermissionSnapshot | null> {
    try {
      ...
      const parsed = JSON.parse(raw) as { snapshot?: string; roleVersions?: Record<string, number> };
  ...
  private async writePermissionSnapshotCache(key: string, value: CachedPermissionSnapshot): Promise<void> {
    try {
